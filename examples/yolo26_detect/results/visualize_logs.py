...
def espdl_preprocess(
    img_bgr: np.ndarray, target_w: int = MODEL_WIDTH, target_h: int = MODEL_HEIGHT
) -> np.ndarray:
    """
    ...
    padded = cv2.copyMakeBorder(
        ...
        pad_right,
        cv2.BORDER_CONSTANT,
        value=(114, 114, 114),  # ESP-DL uses 114 gray pad
    )
        ...
        if current_image:
            ...
            if det_match:
                ...
                results[current_image].append(
                    {"class": cls_name, "score": score, "box": [x1, y1, x2, y2]}
                )
...
def visualize_results(results):
    for img_name, detections in results.items():
        ...
        for i, det in enumerate(detections):
            ...
            x1 = max(0, x1)
            y1 = max(0, y1)
            x2 = min(MODEL_WIDTH, x2)
            y2 = min(MODEL_HEIGHT, y2)
