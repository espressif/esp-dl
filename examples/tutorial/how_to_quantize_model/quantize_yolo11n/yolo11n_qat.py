...
def get_dataset(batchsz=32, imgsz=640, num_workers=8):
    ...
    train_dataset = dataset(
        img_path="coco2017/images/train2017",
        imgsz=imgsz,
        batch_size=batchsz,
        augment=False,
        rect=False,
...
def qat():
    ...
    train_loader = get_dataset(CFG_BATCHSIZE)
    ...
    current_mAP50_95 = trainer.eval()  # before training
    best_mAP50_95 = 0
    for epoch in range(EPOCH):
        ...
        qat_graph = trainer.save(
            os.path.join(str(epoch), "coco_detect_yolo11n_s8_p4_v3.espdl"),
            os.path.join(str(epoch), "coco_detect_yolo11n_s8_p4_v3.native"),
        )
