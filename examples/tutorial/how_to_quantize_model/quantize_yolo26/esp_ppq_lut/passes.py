...
class EspdlLUTFusionPass(QuantizationOptimizationPass):
    ...
    def __init__(
        self,
        target_ops: List[str] = ["Swish", "Sigmoid", "Tanh"],
        verify: bool = False,
        plot: bool = False,
    ...
    def verify_compatibility(self, op: Operation) -> bool:
        ...
        if not hasattr(op, "config") or op.config is None:
            print(
                f"\033[91m[ESPDL Pass] Skipping {op.name}: Operation is not quantized (No TQC found).\033[0m"
            )
            return False
        ...
        if op.input_quant_config[0].num_of_bits != 16:
            print(
                f"\033[91m[ESPDL Pass] Warning: Skipping {op.name} because it is not 16-bit (Found {op.input_quant_config[0].num_of_bits}-bit).\033[0m"
            )
            return False
    ...
    def optimize(self, graph: BaseGraph, **kwargs):
        ...
        for op in graph.operations.values():
            if op.type in self.target_ops:
                ...
                if self.verbose:
                    print(
                        f"[ESPDL Pass] Fused {op.attributes['original_op_type']} -> LUT for operation: {op.name}"
                    )
