...
def run_deep_verification(
    graph, executor, dataloader=None, output_dir="outputs", verbose=False
):
    """
    ...
    if dataloader is not None:
        calib_data = (
            dataloader[0] if isinstance(dataloader, list) else next(iter(dataloader))
        )
    ...
    for i, op in enumerate(lut_ops):
        ...
        if original_op_type not in DEFAULT_BACKEND_TABLE:
            if verbose:
                print(
                    f"[WARNING] Math implementation for '{original_op_type}' not found. Skipping plot."
                )
            continue
        ...
        if os.path.exists(header_out):
            os.remove(header_out)  # New file for each run
