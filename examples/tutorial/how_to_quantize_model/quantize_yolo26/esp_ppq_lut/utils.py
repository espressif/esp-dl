...
def run_numerical_verification(
    ...
    if matches < 100.0:
        layer_info = (
            f" Layer: {op_name} ({op_type})" if op_name and op_type else f" {title}"
        )
        print(
            f"\033[91m[ESPDL Parity Error] Mismatch detected!{layer_info} - Match: {matches:.2f}%\033[0m"
        )
...
def calculate_get_scale(var: Variable, info: ExporterPatternInfo) -> torch.Tensor:
    ...
    return torch.tensor(
        1.0, device=var.value.device if var.value is not None else "cpu"
    )


def calculate_lut_table(
    op: QuantableOperation, info: ExporterPatternInfo, max: int, min: int, step: int = 1
) -> torch.Tensor:
    """
    ...
    if op.type not in platform_dispatching_table:
        print(
            f"\033[91m[CRITICAL ERROR] Operation type '{op.type}' not found in dispatching table for {op.platform.name}.\033[0m"
        )
    ...
    if scale is None:
        print(
            f"\033[91m[ESPDL Patch] Error: Scale for {op.name} is None. Ensure the graph is calibrated.\033[0m"
        )
    ...
    if op.output_quant_config[0].scale is None:
        print(
            f"\033[91m[ESPDL Patch] Error: Output Scale for {op.name} is None.\033[0m"
        )
