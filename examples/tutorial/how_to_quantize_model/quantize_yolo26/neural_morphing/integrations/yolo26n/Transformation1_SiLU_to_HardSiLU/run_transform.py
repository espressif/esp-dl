"""
Transformation 1: SiLU -> HardSiLU (PPQ-native) on YOLO26n
===========================================================
Pipeline:
  1. Export ONNX -> PPQ graph
  2. Fuse Sigmoid+Mul into Swish
  3. Baseline mAP
  4. Morph: Swish -> HardSiluPie8 + Scale (PPQ-native)
  5. Post-morph mAP + report
...
class QATConfig:
    ...
    ESPDL_OUTPUT_DIR = os.path.join(SCRIPT_DIR, "output", f"T1_silu_hsilu_{IMG_SZ}_{PLATFORM}")
...
from esp_ppq.IR import GraphMerger, GraphCommand, GraphCommandType
from esp_ppq.parser import NativeExporter

from neural_morphing.engine import AdaptiveSubgraphOptimizationPass
from silu_to_hardsilu_strategy import SiLUToHardSiLUPPQStrategy
...
NATIVE_CHECKPOINT = os.path.join(QATConfig.ESPDL_OUTPUT_DIR, "morphed_hsilu.native")
...
if __name__ == "__main__":
    ...
    print()
    print("=" * 62)
    print("TRANSFORMATION 1: SiLU -> HardSiLU (PPQ-native)")
    print("=" * 62)
    ...
    # ── PHASE 2: Load PPQ Graph + Fuse Swish ──────────────────
    print("\n  [Phase 2] Loading ONNX → PPQ graph + fusing Swish...")
    ...
    merger.process(GraphCommand(GraphCommandType.FUSE_SWISH))
    graph.topological_sort()

    swish_ops = [n for n, op in graph.operations.items() if op.type == 'Swish']
    print(f"    Swish ops found: {len(swish_ops)}")

    # ── PHASE 3: Baseline mAP ────────────────────────────────
    print("\n  [Phase 3] Baseline mAP (FP32, original Swish)...")
    ...
    print("\n  [Phase 4] Applying Swish → HardSiluPie8 (PPQ-native)...")
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=QATConfig.BATCH_SIZE, shuffle=False
    )
    val_batches = []
    ...
    # Freeze detection head and neck exit layers (INT16 Swish ops
    # that remain as LUT-based activations, not HardSiluPie8).
    frozen_prefixes = [
        "/model.23/",            # detection head
        "/model.16/cv2/",        # neck exit (scale 0)
        "/model.19/cv2/",        # neck exit (scale 1)
        "/model.22/cv2/",        # neck exit (scale 2)
    ]
    print(f"    Freezing layers matching {len(frozen_prefixes)} prefixes")

    strategy = SiLUToHardSiLUPPQStrategy(
        lr=1e-3,
        steps=4000,
        patience=2,
        min_cosine=0.9908,
    ...
    # Post-morph stats
    remaining_swish = [n for n, op in graph.operations.items() if op.type == 'Swish']
    hardsilu_ops = [n for n, op in graph.operations.items() if op.type == 'HardSiluPie8']
    offspring_ops = [n for n in graph.operations
                     if '_parent_op' in graph.operations[n].attributes]
    print(f"\n    Remaining Swish   : {len(remaining_swish)}")
    print(f"    HardSiluPie8 ops  : {len(hardsilu_ops)}")
    print(f"    Total offspring   : {len(offspring_ops)}")

    # ── PHASE 5: Post-morph mAP ──────────────────────────────
    print("\n  [Phase 5] Post-morph mAP (FP32, HardSiluPie8)...")
    ...
    original_count = len(swish_ops)
    decomposed_count = original_count - len(remaining_swish)
    print("\n" + "=" * 62)
    print("RESULTS")
    print("=" * 62)
    print(f"  Baseline mAP50-95  : {baseline_mAP:.4f}")
    ...
    print(f"  Original Swish     : {original_count}")
    print(f"  Replaced (HardSiLU): {decomposed_count}")
    print(f"  Remaining Swish    : {len(remaining_swish)}")
    print(f"  HardSiluPie8 ops   : {len(hardsilu_ops)}")
    print("=" * 62)
