...
from ultralytics.nn.modules import Attention, Detect, Segment
...
class ESP_Segment(Segment):
    def forward(self, x):
        ...
        mc0 = self.cv4[0](x[0])  # mask coefficients
        mc1 = self.cv4[1](x[1])
        mc2 = self.cv4[2](x[2])

        return box0, score0, box1, score1, box2, score2, mc0, mc1, mc2, p
...
class ESP_Segment_Exporter(Exporter):
    """
    adapted from ultralytics for segmentation task
    """
...
class ESP_YOLO(YOLO):
    def export(
        ...
        **kwargs,
        ...
        return ESP_Segment_Exporter(overrides=args, _callbacks=self.callbacks)(
            model=self.model
        )


model = ESP_YOLO("yolo11n-seg.pt")
for m in model.modules():
    if isinstance(m, Attention):
        m.forward = ESP_Attention.forward.__get__(m)
    if isinstance(m, Segment):
        m.forward = ESP_Segment.forward.__get__(m)
