    ...
    namespace base
    {
        ...
        struct ArgsType
        {
            ...
            int16_t *tie_filter_channel_factor; /*<! 26 */
                                                //
            int xtensa_dilation_x_offset;       /*<! 27 (dilation_x * input_channel_with_padding - input_channel) * sizeof(feature_t)*/
            int xtensa_dilation_y_offset;       /*<! 28 */
                                                //
            int tie_conv2d_dilation_x_offset;   /*<! 29 TODO: not used, to be deleted. | dilation_x * input_channel_with_padding * sizeof(feature_t) - (c_div_x_1 + 1) * 16 */
                                                                 ...
                                                                 bool malloc_debug_memory = false)
        {
            ...
            // printf("input: %d, %d, %d, output: %d, %d, %d\n", input.shape[0], input.shape[1], input.shape[2], output.shape[0], output.shape[1], output.shape[2]);

            if (filter.exponent == INT_MIN && sizeof(feature_t) == 1)
            { // S8 per-channel quantization
                args.mac_shift = INT_MIN;

                // calculate scale using filter.channel_exponent
                args.tie_filter_channel_factor = (int16_t *)tool::malloc_aligned(filter.channel_exponent_size, sizeof(int16_t), 16); //TODO: auto_split
                int u = 16 / sizeof(feature_t);
                int len = filter.channel_exponent_size / u * u;

                for (int i = 0; i < len; i++)
                { // special operation for qacc due to cannot shift different per-channel
                    int tmp = output.exponent - filter.channel_exponent[i] - input.exponent;
                    args.tie_filter_channel_factor[i] = (int16_t)1 << (15 - tmp);
                }
                for (int i = len; i < filter.channel_exponent_size; i++)
                { // for conv2d n remainder accx
            ...
            args.xtensa_dilation_y_offset = (args.xtensa_dilation_y_offset_stable - input.shape[2] - (filter.shape[1] - 1) * filter.dilation[1] * input.shape[2]) * sizeof(feature_t);

            args.filter_y_offset_unaligned = 0;
            args.filter_n_offset_unaligned = 0;
            args.filter_element_unaligned = args.n_remainder ? (filter.element + args.n_div_x * args.filter_height * args.filter_width * args.filter_c * u) : filter.element;
            ...
            if (core_number > 1)
            {
                ...
                {
                    m_args[i].input_element = m_args[i - 1].input_element + m_args[i - 1].output_height * args.input_stride_y_offset;
                    m_args[i].output_element = m_args[i - 1].output_element + m_args[i - 1].output_height * args.output_y_offset;
                    m_args[i].output_height = output_y_slice;
                    output_y_remained -= output_y_slice;
                }

                // last slice
                m_args.back().input_element = m_args[core_number - 2].input_element + m_args[core_number - 2].output_height * args.input_stride_y_offset;
                m_args.back().output_element = m_args[core_number - 2].output_element + m_args[core_number - 2].output_height * args.output_y_offset;
                                                    ...
                                                    bool malloc_debug_memory = false)
        {
            ...
            args.input_stride_y_offset = input->shape[1] * input->shape[2] * stride_y;
            args.input_stride_x_offset = input->shape[2] * stride_x;
            args.input_dilation_y_offset = input->shape[1] * input->shape[2] * dilation_y;
            args.input_dilation_x_offset = input->shape[2] * dilation_x;
            ...
            args.xtensa_dilation_x_offset = (dilation_x * input->shape[2] - input->shape[2]) * sizeof(feature_t);
            args.xtensa_dilation_y_offset_stable = dilation_y * input->shape[2] * input->shape[1];
            args.xtensa_dilation_y_offset = (args.xtensa_dilation_y_offset_stable - input->shape[2] - (filter->shape[1] - 1) * dilation_x * input->shape[2]) * sizeof(feature_t);

            args.filter_y_offset_unaligned = 0;
            args.filter_n_offset_unaligned = 0;
            args.filter_element_unaligned = args.n_remainder ? ((feature_t *)args.filter_element + args.n_div_x * args.filter_height * args.filter_width * args.filter_c * u) : args.filter_element;

            if (group > 1) {
                args.filter_w_rs1_1 = (filter->shape[1] >> 1) - 1;
                args.tie_depth2d_dilation_x_offset = dilation_x * input->shape[2] * sizeof(feature_t);
                args.tie_depth2d_dilation_y_offset_stable = dilation_y * input->shape[2] * input->shape[1];
                args.tie_depth2d_dilation_y_offset = (args.tie_depth2d_dilation_y_offset_stable - (filter->shape[1] - 1) * dilation_x * input->shape[2]) * sizeof(feature_t);

                args.tie_depth2d_next_hwx1 = (filter->shape[1] - 1) * dilation_x + (filter->shape[0] - 1) * dilation_y * input->shape[1];
            ...
            if (args.input_height > 4*args.dilation_h*args.filter_height){
                if (runtime_mode == RUNTIME_MODE_MULTI_CORE || (runtime_mode == RUNTIME_MODE_AUTO && args.input_height >= 100 && args.input_width >= 50)) {
                    m_args.push_back(args);

                    // Divide this convolution into two tasks by splitting the input height.
                    // up
                    m_args[0].input_height = (args.input_height - args.input_height / 2) + args.filter_height / 2 * args.dilation_h;
                    m_args[0].padding_h_tail = 0;
                    m_args[0].output_height = (m_args[0].input_height + args.padding_h_head - args.dilation_h * (args.filter_height -1 ) -1) / args.stride_y + 1;
                    //bottom
                    m_args[1].padding_h_head = 0;                    
                    m_args[1].input_height = args.input_height / 2 + args.filter_height / 2 * args.dilation_h;                    
                    m_args[1].input_element += (args.input_height - m_args[1].input_height) * args.input_width * args.input_channel;
                    m_args[1].output_height = (m_args[1].input_height + args.padding_h_tail - args.dilation_h * (args.filter_height -1 ) -1) / args.stride_y + 1;
                    m_args[1].output_element += (args.output_height - m_args[1].output_height)  * args.output_width * args.output_channel;
                }
            }
                                  ...
                                  void (*n_wise_tail)(feature_t *, buffer_t *, const ArgsType<feature_t> &))
        {
            ...
            if (args.padding_h_head || args.padding_w_head || args.padding_h_tail || args.padding_w_tail)
            { // padding same
                ...
                int n_h_body = ((args.input_height + args.padding_h_head - args.dilation_h * (args.filter_height - 1) - 1) / args.stride_y + 1) - n_h_head;
                int n_w_body = ((args.input_width + args.padding_w_head - args.dilation_w * (args.filter_width - 1) - 1) / args.stride_x + 1) - n_w_head;
                ...
                if (i_impl_func_sp)
                {
                    ...
                    if (n_wise_tail)
                    {
                        ...
                        {
                            args.filter_height = filter_h - ((args.padding_h_head - output_y * args.stride_y) + args.dilation_h - 1) / args.dilation_h;
                            input_y_real = input_ptr + args.input_y_offset * ((args.stride_y * output_y + (filter_h - args.filter_height) * args.dilation_h) - args.padding_h_head);
                            filter_ptr_y = filter_ptr + (filter_h - args.filter_height) * filter_w * filter_c_n_ptr_offset;
                            filter_ptr_y_unaligned = filter_ptr_unaligned + (filter_h - args.filter_height) * filter_w * args.filter_c;
                            ...
                            {
                                args.filter_width = filter_w - ((args.padding_w_head - output_x * args.stride_x) + args.dilation_w - 1) / args.dilation_w;
                                input_x_real = input_y_real + args.input_channel * ((args.stride_x * output_x + (filter_w - args.filter_width) * args.dilation_w) - args.padding_w_head);
                                args.filter_y_offset = (filter_w - args.filter_width) * filter_c_n_offset; // ??? c， xtensa， tie 顺序不同
                                args.filter_y_offset_unaligned = (filter_w - args.filter_width) * unaligned_filter_c_n_offset;
                                args.xtensa_dilation_y_offset = (args.xtensa_dilation_y_offset_stable - args.input_channel - (args.filter_width - 1) * args.dilation_w * args.input_channel_with_padding) * sizeof(feature_t);
                                args.filter_element = filter_ptr_y + (filter_w - args.filter_width) * filter_c_n_ptr_offset;
                                args.filter_element_unaligned = filter_ptr_y_unaligned + (filter_w - args.filter_width) * args.filter_c;
                            ...
                            {
                                args.filter_width = (args.padding_w_head + args.input_width - (n_w_head + n_w_body + output_x) * args.stride_x + args.dilation_w - 1) / args.dilation_w;
                                args.filter_y_offset = (filter_w - args.filter_width) * filter_c_n_offset; // ??? c， xtensa， tie 顺序不同
                                args.filter_y_offset_unaligned = (filter_w - args.filter_width) * unaligned_filter_c_n_offset;
                                args.xtensa_dilation_y_offset = (args.xtensa_dilation_y_offset_stable - args.input_channel - (args.filter_width - 1) * args.dilation_w * args.input_channel_with_padding) * sizeof(feature_t);
                                i_impl_func(output_yx, input_x_real, (void *const)&args);
                        ...
                        {
                            args.filter_height = (args.padding_h_head + args.input_height - (n_h_head + n_h_body + output_y) * args.stride_y + args.dilation_h - 1) / args.dilation_h;
                            args.filter_n_offset = (filter_w * (filter_h - args.filter_height)) * filter_c_n_offset;
                            args.filter_n_offset_unaligned = (filter_w * (filter_h - args.filter_height)) * unaligned_filter_c_n_offset;
                                                                   ...
                                                                   bool malloc_debug_memory = false)
        {
            ...
            { // S8 per-channel quantization
                ...
                // calculate scale using filter.channel_exponent
                args.tie_filter_channel_factor = (int16_t *)tool::malloc_aligned(filter.channel_exponent_size, sizeof(int16_t), 16);
                int u = 16 / sizeof(feature_t);
                // int len = filter.channel_exponent_size / u * u;
                //depthwise_conv2d
                for (int i = 0; i < filter.channel_exponent_size; i++)
            ...
            args.tie_depth2d_dilation_y_offset = (args.tie_depth2d_dilation_y_offset_stable - (filter.shape[1] - 1) * filter.dilation[1] * input.shape[2]) * sizeof(feature_t);

            args.tie_depth2d_next_hwx1 = (filter.shape[1] - 1) * filter.dilation[1] + (filter.shape[0] - 1) * filter.dilation[0] * input.shape[1];
            args.tie_depth2d_next_hwx1 = 16 - args.tie_depth2d_next_hwx1 * input.shape[2] * sizeof(feature_t);
                ...
                if (i_impl_func_sp)
                {
                    ...
                    if (n_wise_tail)
                    {
                        for (size_t output_y = 0; output_y < n_h_head; output_y++)
                        {
                            ...
                            input_y_real = input_ptr + args.input_y_offset * ((args.stride_y * output_y + (filter_h - args.filter_height) * args.dilation_h) - args.padding_h_head);
                            filter_ptr_y = filter_ptr + (filter_h - args.filter_height) * filter_w * filter_c_n_ptr_offset;
                            filter_ptr_y_unaligned = filter_ptr_unaligned + (filter_h - args.filter_height) * filter_w * c_remainder_num;
                            args.filter_n_offset = (filter_w * (filter_h - args.filter_height)) * filter_c_n_offset;
                            ...
                            for (size_t output_x = 0; output_x < n_w_head; output_x++)
                            {
                                ...
                                input_x_real = input_y_real + args.input_channel * ((args.stride_x * output_x + (filter_w - args.filter_width) * args.dilation_w) - args.padding_w_head);
                                args.filter_y_offset = (filter_w - args.filter_width + 1) * filter_c_n_offset; // ??? c， xtensa， tie 顺序不同
                                args.filter_y_offset_unaligned = (filter_w - args.filter_width) * unaligned_filter_c_n_offset;
                                args.tie_depth2d_dilation_y_offset = (args.tie_depth2d_dilation_y_offset_stable - (args.filter_width - 1) * args.dilation_w * args.input_channel_with_padding) * sizeof(feature_t);
                                args.tie_depth2d_next_hwx1 = 16 - ((args.filter_width - 1) * args.dilation_w + (args.filter_height - 1) * args.dilation_h * args.input_width) * args.input_channel_with_padding * sizeof(feature_t);
                                args.filter_element = filter_ptr_y + (filter_w - args.filter_width) * filter_c_n_ptr_offset;
                                args.filter_element_unaligned = filter_ptr_y_unaligned + (filter_w - args.filter_width) * c_remainder_num;
        ...
        template <typename feature_t>
        void arith_operation_shell(const arithArgsType<feature_t> &args,
