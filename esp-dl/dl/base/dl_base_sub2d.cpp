...
namespace dl
{
    namespace base
    {
        ...
        inline void sub2d_11c(feature_t *output_ptr, feature_t *input0_ptr, feature_t *input1_ptr, const arithArgsType<feature_t> &args)
        {
            buffer_t buffer;
        ...
        template <typename feature_t, typename buffer_t>
        inline void sub2d_11c_rescale(feature_t *output_ptr, feature_t *input0_ptr, feature_t *input1_ptr, const arithArgsType<feature_t> &args)
        {
            buffer_t buffer;
            ...
            else
            {
                for (size_t output_c = 0; output_c < args.channel; output_c++) // C
                {
                    buffer = (buffer_t)(DL_RIGHT_SHIFT(input1_ptr[output_c], args.input_shift)) - (buffer_t)input0_ptr[output_c];
                    buffer = DL_RIGHT_SHIFT(buffer * args.output_scale, args.output_shift);
                    tool::truncate(output_ptr[output_c], buffer);
                }
            }
                                       ...
                                       const arithArgsType<int16_t> &args)
        {
            ...
            if (args.input0_x_offset % 8 == 0 && args.input1_x_offset % 8 == 0 && args.output_x_offset % 8 == 0 && !((unsigned)&args.input0_element[0] & 15) && !((unsigned)&args.input1_element[0] & 15) && !((unsigned)&args.output_element[0] & 15))
            {
                                      ...
                                      const arithArgsType<int8_t> &args)
        {
            ...
            if (args.input0_x_offset % 16 == 0 && args.input1_x_offset % 16 == 0 && args.output_x_offset % 16 == 0 
                && !((unsigned)&args.input0_element[0] & 15) && !((unsigned)&args.input1_element[0] & 15) && !((unsigned)&args.output_element[0] & 15))
            {
                switch (args.activation_type)
                {
                case Linear:
