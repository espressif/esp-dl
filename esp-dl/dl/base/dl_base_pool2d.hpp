...
#if CONFIG_ESP32P4_BOOST
            args.avg_pool_area_inv = dl_esp32p4_round_half_even(1.f / (args.filter_height * args.filter_width) * (1 << (-args.pool_exponent)));
#else
            args.avg_pool_area_inv = (1 << (-args.pool_exponent)) / (args.filter_height * args.filter_width);
#endif
            int c_div_x = input.shape[2] / u;
            if (args.c_remainder != 0 && args.input_x_offset % u == 0 && args.output_x_offset % u == 0 && !((unsigned)&args.input_element[0] & 15) && !((unsigned)&args.output_element[0] & 15))
            {
                c_div_x += 1;
            }
                            ...
                            void (*c_impl_func)(buffer_t *, feature_t *, feature_t *, PoolArgsType<feature_t> &))
        {
...
#endif
            {
                ...
                for (size_t output_y = 0; output_y < n_h_head; output_y++)
                {
                    ...
                    for (size_t output_x = 0; output_x < n_w_tail; output_x++)
                    {
                        args.filter_width = args.padding_w_head + args.input_width - (n_w_head + n_w_body) * args.stride_x - output_x * args.stride_x;
                        c_impl_func(buffer, input_syx_real, output_yx, args);
                        input_syx_real += args.input_stride_x_offset;
                        output_yx += args.output_x_offset;
                    }
                ...
                for (size_t output_y = 0; output_y < n_h_tail; output_y++)
                {
                    input_syx_real = input_ptr_real;
                    output_yx = output_ptr;
                    args.filter_height = args.padding_h_head + args.input_height - (n_h_head + n_h_body) * args.stride_y - output_y * args.stride_y;
