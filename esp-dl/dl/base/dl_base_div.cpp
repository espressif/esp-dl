...
feature_t *create_div_lut(elemwiseArgsType<feature_t> *args)
{
    ...
    if (sizeof(feature_t) == 1) {
        if (args->input0_d0 == 1) {
            table = (feature_t *)tool::malloc_aligned(256, MALLOC_CAP_DEFAULT);
                                                   ...
                                                   const runtime_mode_t runtime_mode)
{
    ...
    elemwiseArgsType<feature_t> *args =
        (elemwiseArgsType<feature_t> *)tool::malloc_aligned(sizeof(elemwiseArgsType<feature_t>), MALLOC_CAP_DEFAULT);
...
template elemwiseArgsType<int16_t> *get_elemwise_div_args(TensorBase *output,
                                                          ...
                                                          TensorBase *input1,
                                                          const runtime_mode_t runtime_mode);
template elemwiseArgsType<float> *get_elemwise_div_args(TensorBase *output,
                                                        TensorBase *input0,
                                                        TensorBase *input1,
                                                        const runtime_mode_t runtime_mode);
...
template <>
void c_impl_div_n_1<float>(float *output_ptr, float *input0_ptr, float *input1_ptr, void *args)
{
...
template <>
void c_impl_div_1_n<float>(float *output_ptr, float *input0_ptr, float *input1_ptr, void *args)
{
    elemwiseArgsType<float> *elem_args = static_cast<elemwiseArgsType<float> *>(args);
    int32_t length = elem_args->output_d0;

    float input0_float = input0_ptr[0];
    for (int32_t i = 0; i < length; i++) {
        output_ptr[i] = input0_float / input1_ptr[i];
    }
}
...
template <>
void c_impl_div_n_n<float>(float *output_ptr, float *input0_ptr, float *input1_ptr, void *args)
{
    elemwiseArgsType<float> *elem_args = static_cast<elemwiseArgsType<float> *>(args);
    int32_t length = elem_args->output_d0;
...
{
    ImplFunc_t<float, float, float> elemwise_func = c_impl_div_n_n<float>;

    if (args->input1_d0 == 1) {
        elemwise_func = c_impl_div_n_1<float>;
    } else if (args->input0_d0 == 1) {
        elemwise_func = c_impl_div_1_n<float>;
    }
