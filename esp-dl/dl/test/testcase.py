...
class TestDL:
    def __init__(self, target_chip: str, step: int = 25, total: int = 100, quant_method: int = 0):
    ...
    def testcase(self,
        ...
        :return:
        """
        test_unaligned = (True, False)[numpy.random.randint(0, 2)] # actually must choose one
        ...
        activation_type = activation_types[numpy.random.randint(0, len(activation_types))]

        if operation not in ('conv2d', 'depthwise_conv2d') or feature_element_width == 16:
            self.quant_method = 0
        ...
        input_q = numpy.random.randint(feature_low, feature_high, input_shape).astype(float)
        input_f = input_q * 2 ** input_exponent

        # filter
        if self.quant_method == 0:  # per-layer
            filter_exponent = -feature_element_width
            filter_q = numpy.random.randint(feature_low, feature_high, filter_shape).astype(float)
        ...
        output_f = None
        if operation == 'conv2d':
            output_f = tf.nn.conv2d(input_f, filter_f, stride, padding, dilations=dilation)
        elif operation == 'depthwise_conv2d':
            output_f = tf.nn.depthwise_conv2d(input_f, filter_f, stride, padding, dilations=dilation)
        ...
        if operation == 'conv2d':
            output_q = tf.nn.conv2d(input_q, filter_q, stride, padding, dilations=dilation)
        elif operation == 'depthwise_conv2d':
            output_q = tf.nn.depthwise_conv2d(input_q, filter_q, stride, padding, dilations=dilation)
        ...
        if self.quant_method == 1 and feature_element_width == 8 and with_bias:
            ...
            bias_q = numpy.random.randint(bias_low, bias_high, output_channel).astype(int)
            output_q += bias_q
            ...
            if with_bias and 'conv' in operation:
                bias_q = numpy.random.randint(bias_low, bias_high, output_channel).astype(float)
                output_q += bias_q
            output_q = numpy.clip(output_q, feature_low, feature_high)
        ...
        if 'conv' in operation:
            if activation_type == 'ReLU':
                ...
                output_q = numpy.where(output_q < 0, output_q * alpha_q[0] * (2 ** alpha_e), output_q)
                alpha_q = alpha_q.astype(int)
            elif activation_type == 'PReLU':
                alpha_q = numpy.random.randint(feature_low, feature_high, output_channel)
                alpha_e = -feature_element_width
                for i in range(output_channel):
                    output_q[..., i] = numpy.where(output_q[..., i] < 0, output_q[..., i] * alpha_q[i] * 2 ** alpha_e,
                                                   output_q[..., i])
                alpha_q = alpha_q.astype(int)
            else:  # 'Linear'
                pass
        ...
        with open(f'{self.root}/{name}.cpp', 'w') as file:
            ...
            tool.export_element(name='input', array=input_q.astype(int), array_type=feature_type, aligned=aligned)
            tool.export_element(name='output', array=output_q.astype(int), array_type=feature_type, aligned=aligned)

            if 'conv2d' in operation:
                tool(name='layer', operation=operation, feature_type=feature_type,
                     filter_element=filter_q.astype(int), filter_exponent=filter_exponent, stride=stride,
                     dilation=dilation,
            ...
            if self.quant_method == 1 and feature_element_width == 8:
                ...
                bias_str = '&layer_bias' if with_bias else f'(Bias<int{feature_element_width}_t> *)NULL'
            activation_str = '&layer_activation' if activation_type else f'(Activation<int{feature_element_width}_t> *)NULL'
            input_shape_str = input_shape[1:].__str__().replace('(', '{').replace(')', '}')
            filter_shape_str = filter_shape.__str__().replace('(', '{').replace(')', '}')
            if operation.count('conv2d') > 0:
                ...
                file.writelines((
                    ...
                    f'    Tensor<{feature_sign}int{feature_element_width}_t> output = {operation}(input);\n' if operation.count(
                        'max') > 0 else
                    f'    Tensor<{feature_sign}int{feature_element_width}_t> output = {operation}({output_exponent}, input);\n',
                ...
                file.writelines((
                    ...
                    f'    Tensor<{feature_sign}int{feature_element_width}_t> output = {operation}(input, {filter_shape_str}, {stride[1]}, {stride[2]}, {padding_dl});\n' if operation.count(
                        'max') > 0 else
                    f'    Tensor<{feature_sign}int{feature_element_width}_t> output = {operation}({output_exponent}, input, {filter_shape_str}, {stride[1]}, {stride[2]}, {padding_dl});\n',
    ...
    def __call__(self,
        ...
        assert operation in ('conv2d', 'depthwise_conv2d', 'max_pool2d',
                             'avg_pool2d', 'global_max_pool2d', 'global_avg_pool2d')
        ...
        for _ in range(self.total):
            ...
            if operation == 'conv2d':
                if filter_shape is None:
                    _filter_shape = tuple(numpy.random.randint(1, 8, 2)) + tuple(numpy.random.randint(1, 48, 2))
                ...
                if filter_shape is None:
                    _filter_shape = tuple(numpy.random.randint(1, 8, 2)) + (numpy.random.randint(1, 256), 1)
                elif len(filter_shape) == 2:
                    _filter_shape = filter_shape + tuple(numpy.random.randint(1, 256, 1)) + (1,)
                elif len(filter_shape) == 4:
                    _filter_shape = filter_shape
                else:
                    raise ValueError(f'filter_shape can not be {filter_shape}.')
            ...
            if operation in ('global_max_pool2d', 'global_avg_pool2d'):
                ...
                input_shape_min = max((_filter_shape[0] - 1) * _dilation[0] + 1,
                                      (_filter_shape[1] - 1) * _dilation[1] + 1)
                input_shape_max = max(((_filter_shape[0] - 1) * _dilation[0] + 1) * _stride[1],
                                      ((_filter_shape[1] - 1) * _dilation[1] + 1) * _stride[2]) * 2
...
if __name__ == '__main__':
    ...
    parser.add_argument('--activation', help='\"(None, \\"ReLU\\", \\"LeakyReLU\\", \\"PReLU\\")\"', default='None')
    parser.add_argument('--step', help='Wait for every this number of testcases', default='20')
    parser.add_argument('--total', help='The total of testcases', default='100')
    parser.add_argument('--quant', help='The quantization method of filter', default='0')
    args = parser.parse_args()

    if all((args.target_chip, args.operation)):
        test_dl = TestDL(target_chip=args.target_chip, step=eval(args.step), total=eval(args.total),
                         quant_method=eval(args.quant))
        test_dl(operation=args.operation,
