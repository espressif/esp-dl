...
template <typename feature_t>
Tensor<feature_t> mul2d_c(const int output_exponent, Tensor<feature_t> &input0, Tensor<feature_t> &input1, const Activation<feature_t> *activation, const std::vector<int> &assign_core = CONFIG_DEFAULT_ASSIGN_CORE)
{
...
bool test_mul_s8(int exponent0, int exponent1, int exponent_out, int offset0, int offset1, int height, int width, int channel, int activation_type, bool inplace)
{
	if (!inplace)
	{
		...
		if (activation_type == 0)
		{
			...
			latency.start();
			Tensor<int8_t> output = mul2d<false, int8_t>(exponent_out, input0, input1, (Activation<int8_t> *)NULL); //(Activation<int8_t> *)NULL
			latency.end();
			latency.print();
...
bool test_mul_s16(int exponent0, int exponent1, int exponent_out, int offset0, int offset1, int height, int width, int channel, int activation_type, bool inplace)
{
	if (!inplace)
	{
		...
		if (activation_type == 0)
		{
			...
			latency.start();
			Tensor<int16_t> output = mul2d<false, int16_t>(exponent_out, input0, input1, (Activation<int16_t> *)NULL); //(Activation<int8_t> *)NULL
			latency.end();
			latency.print();
	...
	else
	{
		...
		if (activation_type == 0)
		{
			Tensor<int16_t> output_c = mul2d_c(exponent_out, input0, input1, (Activation<int16_t> *)NULL);
			latency.start();
			mul2d<true, int16_t>(exponent_out, input0, input1, (Activation<int16_t> *)NULL); //(Activation<int8_t> *)NULL
			latency.end();
			latency.print();
