			 ...
			 const Activation<feature_t> *const activation)
{
	...
	for (size_t output_y = 0; output_y < height; output_y++)
	{
		...
		for (size_t output_x = 0; output_x < width; output_x++)
		{
			...
			{
				buffer = (int)(DL_RIGHT_SHIFT(input0_11c[output_c], input0_shift)) - (int)(DL_RIGHT_SHIFT(input1_11c[output_c], input1_shift));
...
template <typename feature_t>
Tensor<feature_t> sub2d_c(const int output_exponent, Tensor<feature_t> &input0, Tensor<feature_t> &input1, const Activation<feature_t> *activation, const std::vector<int> &assign_core = CONFIG_DEFAULT_ASSIGN_CORE)
{
...
bool test_sub_s8(int exponent0, int exponent1, int exponent_out, int offset0, int offset1, int height, int width, int channel, int activation_type, bool inplace)
{
	if (!inplace)
	{
		Tensor<int8_t> input0;
		input0.set_element((int8_t *)&input0_element[offset0]).set_exponent(exponent0).set_shape({height, width, channel}).set_auto_free(false);

		Tensor<int8_t> input1;
		input1.set_element((int8_t *)&input1_element[offset1]).set_exponent(exponent1).set_shape({height, width, channel}).set_auto_free(false);
		Latency latency;
		...
		if (activation_type == 0)
		{
			...
			latency.start();
			Tensor<int8_t> output = sub2d<false, int8_t>(exponent_out, input0, input1, (Activation<int8_t> *)NULL); //(Activation<int8_t> *)NULL
			latency.end();
			latency.print();
	...
	else
	{
		...
		input0_tmp.set_element((int8_t *)&input0_element[offset0]).set_exponent(exponent0).set_shape({height, width, channel}).set_auto_free(false);

		Tensor<int8_t> input1_tmp;
		input1_tmp.set_element((int8_t *)&input1_element[offset1]).set_exponent(exponent1).set_shape({height, width, channel}).set_auto_free(false);
		Latency latency;
...
bool test_sub_s16(int exponent0, int exponent1, int exponent_out, int offset0, int offset1, int height, int width, int channel, int activation_type, bool inplace)
{

	if (!inplace)
	{
		Tensor<int16_t> input0;
		input0.set_element((int16_t *)&input0_element_s16[offset0]).set_exponent(exponent0).set_shape({height, width, channel}).set_auto_free(false);

		Tensor<int16_t> input1;
		input1.set_element((int16_t *)&input1_element_s16[offset1]).set_exponent(exponent1).set_shape({height, width, channel}).set_auto_free(false);
		Latency latency;
		...
		if (activation_type == 0)
		{
			...
			latency.start();
			Tensor<int16_t> output = sub2d<false, int16_t>(exponent_out, input0, input1, (Activation<int16_t> *)NULL); //(Activation<int8_t> *)NULL
			latency.end();
			latency.print();
	...
	else
	{
		...
		input0_tmp.set_element((int16_t *)&input0_element_s16[offset0]).set_exponent(exponent0).set_shape({height, width, channel}).set_auto_free(false);

		Tensor<int16_t> input1_tmp;
		input1_tmp.set_element((int16_t *)&input1_element_s16[offset1]).set_exponent(exponent1).set_shape({height, width, channel}).set_auto_free(false);
		Latency latency;
		...
		if (activation_type == 0)
		{
			Tensor<int16_t> output_c = sub2d_c(exponent_out, input0, input1, (Activation<int16_t> *)NULL);
			latency.start();
			sub2d<true, int16_t>(exponent_out, input0, input1, (Activation<int16_t> *)NULL); //(Activation<int8_t> *)NULL
			latency.end();
			latency.print();
