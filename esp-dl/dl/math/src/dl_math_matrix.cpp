...
namespace dl
{
    namespace math
    {
        ...
        Matrix<float> get_affine_transform(Matrix<float> &source_coord, Matrix<float> &dest_coord)
        {
            Matrix<float> m(3, 3);
            float Ainv[3][3] = {0};
            float Adet = (source_coord.array[0][0] * source_coord.array[1][1] + source_coord.array[0][1] * source_coord.array[2][0] +
            ...
            Ainv[2][0] = (source_coord.array[1][0] * source_coord.array[2][1] - source_coord.array[2][0] * source_coord.array[1][1]) / Adet;
            Ainv[2][1] = (source_coord.array[2][0] * source_coord.array[0][1] - source_coord.array[0][0] * source_coord.array[2][1]) / Adet;
            Ainv[2][2] = (source_coord.array[0][0] * source_coord.array[1][1] - source_coord.array[0][1] * source_coord.array[1][0]) / Adet;

            for (int i = 0; i < 3; i++)
            {
                m.array[0][i] = Ainv[i][0] * dest_coord.array[0][0] + Ainv[i][1] * dest_coord.array[1][0] + Ainv[i][2] * dest_coord.array[2][0];
                m.array[1][i] = Ainv[i][0] * dest_coord.array[0][1] + Ainv[i][1] * dest_coord.array[1][1] + Ainv[i][2] * dest_coord.array[2][1];
            }
            m.array[2][0] = 0;
            m.array[2][1] = 0;
            m.array[2][2] = 1;
        ...
        Matrix<float> get_similarity_transform(Matrix<float> &source_coord, Matrix<float> &dest_coord)
        {
            ...
            double l1 = (AAT[0][0] + AAT[1][1] + sqrt_newton((AAT[0][0] + AAT[1][1]) * (AAT[0][0] + AAT[1][1]) - 4 * ((AAT[0][0] * AAT[1][1]) - (AAT[0][1] * AAT[1][0])))) / 2.0;
            double l2 = (AAT[0][0] + AAT[1][1] - sqrt_newton((AAT[0][0] + AAT[1][1]) * (AAT[0][0] + AAT[1][1]) - 4 * ((AAT[0][0] * AAT[1][1]) - (AAT[0][1] * AAT[1][0])))) / 2.0;
            S[0] = sqrt_newton(l1);
            S[1] = sqrt_newton(l2);
        ...
        Matrix<float> get_perspective_transform(Matrix<float> &source_coord, Matrix<float> &dest_coord)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                m.array[i / 3][i % 3] = (((Ainv.array[i][0]) * dest_coord.array[0][0]) + ((Ainv.array[i][1]) * dest_coord.array[1][0]) +
