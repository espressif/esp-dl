...
namespace dl
{
    ...
    void Tensor<T>::print(std::vector<int> axis_index_range, const char *message)
    {
        ...
        {
            axis_index_range[2 * i] = (axis_index_range[2 * i] < 0) ? (this->shape[i] + axis_index_range[2 * i]) : axis_index_range[2 * i];
            axis_index_range[2 * i + 1] = (axis_index_range[2 * i + 1] < 0) ? (this->shape[i] + axis_index_range[2 * i + 1]) : axis_index_range[2 * i + 1];
        ...
        axis_index_range[2 * last_i] = (axis_index_range[2 * last_i] < 0) ? (this->shape[last_i] + axis_index_range[2 * last_i]) : axis_index_range[2 * last_i];
        axis_index_range[2 * last_i + 1] = (axis_index_range[2 * last_i + 1] < 0) ? (this->shape[last_i] + axis_index_range[2 * last_i + 1]) : axis_index_range[2 * last_i + 1];
    ...
    Tensor<T> &Tensor<T>::set_value(std::vector<int> axis_index_range, T value)
    {
        ...
        for (int i = 0; i < dims; ++i)
        {
            loop_index_lower_bound[i] = axis_index_range[2 * i] < 0 ? this->shape[i] + axis_index_range[2 * i] : axis_index_range[2 * i];
            loop_index_lower_bound[i] = loop_index_lower_bound[i] < 0 ? 0 : loop_index_lower_bound[i];
            loop_index_upper_bound[i] = axis_index_range[2 * i + 1] < 0 ? this->shape[i] + axis_index_range[2 * i + 1] : axis_index_range[2 * i + 1];
            loop_index_upper_bound[i] = loop_index_upper_bound[i] > this->shape[i] ? this->shape[i] : loop_index_upper_bound[i];
                ...
                if (std::is_same<T, int8_t>::value) {         // int8 -> int8
                    ...
                    if (exponent == 0) {
                        ...
                        for (int i=0; i<this->get_size(); i++) {
                            element[i] = quantize(dequantize(in_element[i], DL_SCALE(input.exponent)), scale, DL_QUANT8_MIN, DL_QUANT8_MAX);
                        }
                    }
                    ...
                    if (exponent == 0) {
                        ...
                        float scale = 1.0 / (DL_SCALE(this->exponent));
                        for (int i=0; i<this->get_size(); i++) {
                            element[i] = quantize(dequantize(in_element[i], DL_SCALE(input.exponent)), scale, DL_QUANT16_MIN, DL_QUANT16_MAX);
                        }
                    }
