...
public:
    /**
     * @brief Construct a new GreaterOrEqual object.
     *
    ...
    /**
     * @brief Destroy the GreaterOrEqual object.
     */
    ~GreaterOrEqual() {}
    ...
    void forward(std::vector<dl::TensorBase *> &tensors, runtime_mode_t mode)
    {
        ...
        DL_LOG_MODULE_LATENCY_END_PRINT(this->name, "GreaterOrEqual");
    }

    void forward_args(void *args)
    {
        if (quant_type == QUANT_TYPE_SYMM_8BIT) {
            base::elemwise_greaterorequal((base::elemwiseArgsType<int8_t, bool> *)args);
        } else if (quant_type == QUANT_TYPE_SYMM_16BIT) {
            base::elemwise_greaterorequal((base::elemwiseArgsType<int16_t, bool> *)args);
        }
    }
    ...
    void forward_template(std::vector<TensorBase *> &tensors, runtime_mode_t mode)
    {
        ...
        if (task_size == 1) { // single task
            ...
            ESP_LOGE("GreaterOrEqual", "Only support task size is 1 or 2, currently task size is %d", task_size);
        }
    }
