...
public:
    ...
    /**
     * @brief Destroy the ReduceSum object.
     */
    ~ReduceSum() {}
                    ...
                    void *arg)
    {
        ...
        if constexpr (std::is_same<T, float>::value && std::is_same<V_T, float>::value) {
            // Directly return the sum for float32
            return ReduceBase::reduce<reduce_op_add<V_T, T>>(v0, ptr, size0, stride0, size1, stride1, arg);
        } else {
    ...
    /**
     * @brief deserialize ReduceSum module instance by node serialization information
     */
    static Module *deserialize(fbs::FbsModel *fbs_model, std::string node_name)
    {
        ...
        // Create module
        op = new ReduceSum(
            keepdims, axes_reduce_flag, "ReduceSum", node_name.c_str(), MODULE_INPLACE_CHANGED_BUFFER, quant_type);
        return op;
    }

    void print() { ReduceBase::print("ReduceSum"); }
};
} // namespace module
} // namespace dl
