...
public:
        ...
        Module(name, inplace, quant_type), m_hidden_size(hidden_size), m_direction(direction), m_layout(layout)
    {
        ...
        if (quant_type == QUANT_TYPE_SYMM_8BIT || quant_type == QUANT_TYPE_SYMM_16BIT) {
            ...
            m_sigmoid_lut = new math::SigmoidLUT(-8, -6.3, 6.3, caps); // +-6.3 is the boundary for sigmoid
            m_tanh_lut = new math::TanhLUT(-8, -3.5, 3.5, caps);       // +-3.5 is the boundary for tanh
            m_gate_exponent = -8; // fixed exponent for gate calculations, because of the LUT range
    ...
    void forward_template(ModelContext *context, runtime_mode_t mode)
    {
        ...
        if (m_first_step) {
            ...
            if (m_inputs_index.size() > 5) {
                ...
                initial_h->print();
                m_h_prev->assign(initial_h);
            } else {
                // Initialize h_prev to zeros
                memset(m_h_prev->get_element_ptr(), 0, m_h_prev->get_size());
            }
        ...
        float out_scale = DL_RESCALE(output_y->get_exponent());
        ...
        for (int seq = 0; seq < seq_length; seq++) {
            // uint32_t c1= esp_timer_get_time();

            // W * x_t
            base::mat_vec_dotprod(w_items, x_items, input_cache_items, 3 * m_hidden_size, input_size, w_shift);

            // R * h_prev
            base::mat_vec_dotprod(r_items, h_prev_items, hidden_cache_items, 3 * m_hidden_size, m_hidden_size, r_shift);
            // uint32_t c2= esp_timer_get_time();
            // printf("----> dotprod:%ld\n", c2-c1);
            ...
            if (quant_type == QUANT_TYPE_SYMM_8BIT) {
                for (int i = 0; i < m_hidden_size; i++) {
                    float temp = (1 - z_gate[i]) * n_gate[i] * out_scale;
                    temp += z_gate[i] * h_prev_items[i];
                    tool::truncate(h_prev_items[i], tool::round(temp));
                    tool::truncate(y_items[i], h_prev_items[i]);
                }
            } else {
                for (int i = 0; i < m_hidden_size; i++) {
                    float temp = (1 - z_gate[i]) * n_gate[i] * out_scale;
        ...
        memcpy(h_items, y_items - m_hidden_size, m_hidden_size * sizeof(T));
    }
    ...
    static Module *deserialize(fbs::FbsModel *fbs_model, std::string node_name)
    {
        ...
        if (direction == "bidirectional") {
            ESP_LOGE("GRU", "Only forward GRU is supported.");
            return op;
        }
