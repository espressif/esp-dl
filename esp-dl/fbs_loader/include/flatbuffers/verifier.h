...
public:
    ...
    template <typename offset = uoffset_t, typename SOffsetT = soffset_t>
    size_t VerifyOffset(const size_t start) const
    {
        if (!Verify<offset>(start))
            return 0;
        const auto o = ReadScalar<offset>(buf_ + start);
    ...
    template <typename offset = uoffset_t>
    size_t VerifyOffset(const uint8_t *const base, const voffset_t start) const
    {
        return VerifyOffset<offset>(static_cast<size_t>(base - buf_) + start);
    }
