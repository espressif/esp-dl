...
inline void IndentString(std::string &s, int indent, const char *indent_string)
{
    for (int i = 0; i < indent; i++) s += indent_string;
}

template <typename T>
void AppendToString(std::string &s, T &&v, bool keys_quoted, bool indented, int cur_indent, const char *indent_string)
{
    s += "[";
    s += indented ? "\n" : " ";
...
public:
    ...
    void ToString(bool strings_quoted,
                  bool keys_quoted,
                  std::string &s,
                  bool indented,
                  int cur_indent,
                  const char *indent_string) const
    {
        if (type_ == FBT_STRING) {
            ...
            AppendToString<Vector>(s, AsVector(), keys_quoted, indented, cur_indent + 1, indent_string);
        } else if (IsTypedVector()) {
            AppendToString<TypedVector>(s, AsTypedVector(), keys_quoted, indented, cur_indent + 1, indent_string);
        } else if (IsFixedTypedVector()) {
            AppendToString<FixedTypedVector>(
                s, AsFixedTypedVector(), keys_quoted, indented, cur_indent + 1, indent_string);
        } else if (IsBlob()) {
            auto blob = AsBlob();
...
public:
    ...
    // Reset all state so we can reuse the buffer.
    void Clear()
    {
        buf_.clear();
        stack_.clear();
