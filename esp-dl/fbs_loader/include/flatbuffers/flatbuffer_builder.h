...
public:
    ...
    template <typename T, template <typename> class offset = Offset>
    ...
    template <template <typename> class offset = Offset>
    offset<String> CreateString(const char *str, size_t len)
    {
        CreateStringImpl(str, len);
        return offset<String>(CalculateOffset<typename offset<String>::offset_type>());
    }
    ...
    {
        return CreateString<offset>(str, strlen(str));
    }
    ...
    {
        return CreateString<offset>(str.c_str(), str.length());
    }
  ...
  offset<String>CreateString(flatbuffers::string_view str) {
    return CreateString<offset>(str.data(), str.size());
  }
    ...
    {
        return str ? CreateString<offset>(str->c_str(), str->size()) : 0;
    }
    ...
    {
        return CreateString<offset>(str.data(), str.length());
    }
    ...
    template <template <typename> class offset = Offset, typename LenT = uint32_t>
    ...
    template <typename T, template <typename> class offset = Offset, typename LenT = uint32_t>
    void StartVector(size_t len)
    {
        return StartVector<offset, LenT>(len, sizeof(T), AlignOf<T>());
    }
    ...
    /// @tparam offset the type of offset to return
    ...
    template <typename T, template <typename...> class offset = Offset, template <typename...> class VectorT = Vector>
    offset<VectorT<T>> CreateVector(const T *v, size_t len)
    {
        // The type of the length field in the vector.
        typedef typename VectorT<T>::size_type LenT;
        typedef typename offset<VectorT<T>>::offset_type offset_type;
        // If this assert hits, you're specifying a template argument that is
        // causing the wrong overload to be selected, remove it.
        AssertScalarT<T>();
        StartVector<T, offset, LenT>(len);
        if (len > 0) {
            // clang-format off
        ...
        return offset<VectorT<T>>(EndVector<LenT, offset_type>(len));
    }
    ...
    /// where the vector is stored.
    template <typename T, template <typename...> class offset = Offset, template <typename...> class VectorT = Vector>
    offset<VectorT<const T *>> CreateVectorOfStructs(const T *v, size_t len)
    {
        // The type of the length field in the vector.
        typedef typename VectorT<T>::size_type LenT;
        typedef typename offset<VectorT<const T *>>::offset_type offset_type;

        StartVector<offset, LenT>(len, sizeof(T), AlignOf<T>());
        if (len > 0) {
            PushBytes(reinterpret_cast<const uint8_t *>(v), sizeof(T) * len);
        }
        return offset<VectorT<const T *>>(EndVector<LenT, offset_type>(len));
    }
    ...
    /// where the vector is stored.
    template <typename T,
              template <typename...> class offset = Offset,
              template <typename...> class VectorT = Vector,
              typename Alloc = std::allocator<T>>
    offset<VectorT<const T *>> CreateVectorOfStructs(const std::vector<T, Alloc> &v)
    {
        return CreateVectorOfStructs<T, offset, VectorT>(data(v), v.size());
    }
...
private:
    ...
    {
        StartVector<offset>(vector_size, sizeof(T), AlignOf<T>());
    ...
    {
        return offset<Vector<const T *>>(
            EndVector<typename Vector<const T *>::size_type, typename offset<Vector<const T *>>::offset_type>(
                vector_size));
    }
