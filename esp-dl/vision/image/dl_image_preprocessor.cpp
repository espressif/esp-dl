...
void ImagePreprocessor<feature_t>::create_norm_lut()
{
    for (int i = 0; i < this->mean.size(); i++) {
        if (std::is_same<feature_t, int8_t>::value) {
            for (int j = 0; j < 256; j++) {
                this->norm_lut[i * 256 + j] =
                    (feature_t)DL_CLIP(dl_esp32p4_round_half_even(((float)j - this->mean[i]) / this->std[i] /
                                                                  DL_SCALE(this->model_input->exponent)),
                                       -128,
                                       127);
            }
        } else {
            for (int j = 0; j < 65536; j++) {
                this->norm_lut[i * 65536 + j] =
                    (feature_t)DL_CLIP(dl_esp32p4_round_half_even(((float)j - this->mean[i]) / this->std[i] /
                                                                  DL_SCALE(this->model_input->exponent)),
                                       -32768,
                                       32767);
...
template <typename feature_t>
void ImagePreprocessor<feature_t>::preprocess(uint8_t *input_element,
                                              ...
                                              const std::vector<int> &crop_area)
{
    ...
    if (!crop_area.empty() || input_shape[0] != this->model_input->shape[1] ||
        ...
        if (CONFIG_IDF_TARGET_ESP32P4 && this->resize_scale_y < 256 && this->resize_scale_x < 256 &&
            this->resize_scale_y >= 0.0625 && this->resize_scale_x >= 0.0625 && this->use_ppa) {
...
template <typename feature_t>
void ImagePreprocessor<feature_t>::preprocess(uint16_t *input_element,
                                              const std::vector<int> &input_shape,
                                              const std::vector<int> &crop_area)
{
    dl::tool::Latency latency;
