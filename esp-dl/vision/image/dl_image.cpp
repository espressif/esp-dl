...
namespace dl
{
    namespace image
    {
        ...
        template void crop_and_resize(uint8_t *dst_image, int dst_width, int dst_channel, int dst_y_start, int dst_y_end, int dst_x_start, int dst_x_end, uint16_t *src_image, int src_height, int src_width, int src_channel, int src_y_start, int src_y_end, int src_x_start, int src_x_end, resize_type_t resize_type, bool rgb_swap, bool byte_swap);
                ...
                for (size_t y = dst_y_start; y < dst_y_end; y++)
                {
                    ...
                    for (size_t x = dst_x_start; x < dst_x_end; x++)
                    {
                        ...
                        if (src_channel == dst_channel)
                        {
                            ...
                            {
                                temp = (int)src_image[src_row_0 + c] + (int)src_image[src_row_0 + dst_channel + c] + (int)src_image[src_row_1 + c] + (int)src_image[src_row_1 + dst_channel + c];
                                if (rgb_swap)
            ...
            case IMAGE_RESIZE_NEAREST:
                for (size_t y = dst_y_start; y < dst_y_end; y++)
                {
                    ...
                    for (size_t x = dst_x_start; x < dst_x_end; x++)
                    {
                        ...
                        {
                            if (rgb_swap)
                                dst_image[dst_i] = convert_pixel_rgb888_to_gray(src_image[src_i], src_image[src_i + 1], src_image[src_i + 2]);
                            else
                                dst_image[dst_i] = convert_pixel_rgb888_to_gray(src_image[src_i + 2], src_image[src_i + 1], src_image[src_i]);
                        }
                        else
                        {
                            printf("Not implement src_channel = %d and dst_channel = %d\n", src_channel, dst_channel);
        ...
        template void crop_and_resize(uint8_t *dst_image, int dst_width, int dst_channel, int dst_y_start, int dst_y_end, int dst_x_start, int dst_x_end, uint8_t *src_image, int src_height, int src_width, int src_channel, int src_y_start, int src_y_end, int src_x_start, int src_x_end, resize_type_t resize_type, bool rgb_swap);
        template void crop_and_resize(int16_t *dst_image, int dst_width, int dst_channel, int dst_y_start, int dst_y_end, int dst_x_start, int dst_x_end, uint8_t *src_image, int src_height, int src_width, int src_channel, int src_y_start, int src_y_end, int src_x_start, int src_x_end, resize_type_t resize_type, bool rgb_swap);
        template void crop_and_resize(int8_t *dst_image, int dst_width, int dst_channel, int dst_y_start, int dst_y_end, int dst_x_start, int dst_x_end, uint8_t *src_image, int src_height, int src_width, int src_channel, int src_y_start, int src_y_end, int src_x_start, int src_x_end, resize_type_t resize_type, bool rgb_swap);
                        ...
                        const uint32_t color)
        {
            ...
            draw_filled_rectangle(image, image_height, image_width,
                                  DL_MAX((int)x - half_size, 0), DL_MAX((int)y - half_size, 0), x + half_size, y + half_size,
                                  color);
        }
        ...
        uint32_t get_moving_point_number(uint16_t *f1, uint16_t *f2, const uint32_t height, const uint32_t width, const uint32_t stride, const uint32_t threshold)
        {
        ...
        uint32_t get_moving_point_number(uint8_t *f1, uint8_t *f2, const uint32_t height, const uint32_t width, const uint32_t stride, const uint32_t threshold)
        {
        ...
        void warp_affine(dl::Tensor<T> *input, dl::Tensor<T> *output, dl::math::Matrix<float> *M_inv)
        {
            ...
            T *dst = output->get_element_ptr();
            T *src = input->get_element_ptr();
            ...
            for (int i = 0; i < output_h; i++)
            {
                ...
                {
                    x_src = (M_inv->array[0][0] * j + M_inv->array[0][1] * i + M_inv->array[0][2]) / (M_inv->array[2][0] * j + M_inv->array[2][1] * i + M_inv->array[2][2]);
                    y_src = (M_inv->array[1][0] * j + M_inv->array[1][1] * i + M_inv->array[1][2]) / (M_inv->array[2][0] * j + M_inv->array[2][1] * i + M_inv->array[2][2]);
                    ...
                    else
                    {
                        ...
                        for (int k = 0; k < c; k++)
                        {
                            *dst++ = (T)rintf(((src[y1 * input_stride + x1 * c + k]) * (x2 - x_src) * (y2 - y_src)) + ((src[y1 * input_stride + x2 * c + k]) * (x_src - x1) * (y2 - y_src)) + ((src[y2 * input_stride + x1 * c + k]) * (x2 - x_src) * (y_src - y1)) + ((src[y2 * input_stride + x2 * c + k]) * (x_src - x1) * (y_src - y1)));
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < output_h; i++)
            {
                for (int j = 0; j < output_w; j++)
                {
                    ...
                    else
                    {
                        ...
                        *dst++ = (T)rintf((src_x1y1[0] * (x2 - x_src) * (y2 - y_src)) + (src_x2y1[0] * (x_src - x1) * (y2 - y_src)) + (src_x1y2[0] * (x2 - x_src) * (y_src - y1)) + (src_x2y2[0] * (x_src - x1) * (y_src - y1)));
                        *dst++ = (T)rintf((src_x1y1[1] * (x2 - x_src) * (y2 - y_src)) + (src_x2y1[1] * (x_src - x1) * (y2 - y_src)) + (src_x1y2[1] * (x2 - x_src) * (y_src - y1)) + (src_x2y2[1] * (x_src - x1) * (y_src - y1)));
                        *dst++ = (T)rintf((src_x1y1[2] * (x2 - x_src) * (y2 - y_src)) + (src_x2y1[2] * (x_src - x1) * (y2 - y_src)) + (src_x1y2[2] * (x2 - x_src) * (y_src - y1)) + (src_x2y2[2] * (x_src - x1) * (y_src - y1)));
                    }
                }
            }
        }
        template void warp_affine(uint16_t *input, std::vector<int> shape, dl::Tensor<uint8_t> *output, dl::math::Matrix<float> *M_inv);
        template void warp_affine(uint16_t *input, std::vector<int> shape, dl::Tensor<int16_t> *output, dl::math::Matrix<float> *M_inv);
        ...
        Tensor<uint8_t> *gen_binary_img(Tensor<uint8_t> &image, std::vector<int> thresh)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if (((*c1) >= thresh[0]) && ((*c1) <= thresh[1]) && ((*c2) >= thresh[2]) && ((*c2) <= thresh[3]) && ((*c3) >= thresh[4]) && ((*c3) <= thresh[5]))
                {
                    *(pbin++) = 255;
                    // num++;
                }
                ...
                if (resize_type == IMAGE_RESIZE_BILINEAR)
                {
                    for (int h = 0; h < target_shape[0]; ++h)
                    {
                        ...
                        for (int w = 0; w < target_shape[1]; ++w)
                        {
                            ...
                            resized_ptr[0] = (uint8_t)(image.get_element_value({h1, w1, 0}) * h1_weight * w1_weight + image.get_element_value({h1, w2, 0}) * h1_weight * w2_weight + image.get_element_value({h2, w1, 0}) * h2_weight * w1_weight + image.get_element_value({h2, w2, 0}) * h2_weight * w2_weight);
                            ...
                            resized_ptr[2] = (uint8_t)(image.get_element_value({h1, w1, 2}) * h1_weight * w1_weight + image.get_element_value({h1, w2, 2}) * h1_weight * w2_weight + image.get_element_value({h2, w1, 2}) * h2_weight * w1_weight + image.get_element_value({h2, w2, 2}) * h2_weight * w2_weight);
                            resized_ptr += 3;
                        }
                    }
                ...
                else if (resize_type == IMAGE_RESIZE_MEAN)
                {
                    for (int h = 0; h < target_shape[0]; ++h)
                    {
                        ...
                        for (int w = 0; w < target_shape[1]; ++w)
                        {
                            ...
                            int w1 = (int)w_origin;
                            int w2 = w1 + 1;
                            resized_ptr[0] = (uint8_t)(((int)image.get_element_value({h1, w1, 0}) + (int)image.get_element_value({h1, w2, 0}) + (int)image.get_element_value({h2, w1, 0}) + (int)image.get_element_value({h2, w2, 0})) >> 2);
                            resized_ptr[1] = (uint8_t)(((int)image.get_element_value({h1, w1, 1}) + (int)image.get_element_value({h1, w2, 1}) + (int)image.get_element_value({h2, w1, 1}) + (int)image.get_element_value({h2, w2, 1})) >> 2);
                            resized_ptr[2] = (uint8_t)(((int)image.get_element_value({h1, w1, 2}) + (int)image.get_element_value({h1, w2, 2}) + (int)image.get_element_value({h2, w1, 2}) + (int)image.get_element_value({h2, w2, 2})) >> 2);
                            resized_ptr += 3;
                        }
                    }
                    return resized_image;
        ...
        template void resize_image_nearest(int32_t *image, std::vector<int> input_shape, int32_t *resized_image, std::vector<int> target_shape);
        template void resize_image_nearest(uint32_t *image, std::vector<int> input_shape, uint32_t *resized_image, std::vector<int> target_shape);
        template void resize_image_nearest(int16_t *image, std::vector<int> input_shape, int16_t *resized_image, std::vector<int> target_shape);
        template void resize_image_nearest(uint16_t *image, std::vector<int> input_shape, uint16_t *resized_image, std::vector<int> target_shape);
        template void resize_image_nearest(int8_t *image, std::vector<int> input_shape, int8_t *resized_image, std::vector<int> target_shape);
        template void resize_image_nearest(uint8_t *image, std::vector<int> input_shape, uint8_t *resized_image, std::vector<int> target_shape);
        template void resize_image_nearest(float *image, std::vector<int> input_shape, float *resized_image, std::vector<int> target_shape);
