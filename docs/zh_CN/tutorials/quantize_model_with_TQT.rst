...
在采用 TQT 策略对 8-bit 模型进行量化优化的情况下，模型的 Top-1 准确率为 71.775%，相比 ESP32P4 本身的 per-channel 的量化模型（Top-1 准确率为 70.850%），TQT 在 Top-1 精度上提升了 1.306 个百分点, 与float32模型的精度（71.878%）更加接近，表明即使在严格的 8-bit 量化约束下，仅通过对量化阈值进行可学习优化，TQT 也能够有效缓解量化误差带来的性能下降，使量化模型的精度接近浮点模型水平。
