...
def gen_op_create_prompt(
    op_type: str, op_info: str, ref_ops: list[str], support_dtype="int8,int16,float", target="esp32p4"
) -> str:
...
- If torcg has a {op_type} interface, modify `tools/ops_test/torch_ops_test.py` to add {op_type} tests, Use torch {op_type} interface for implementation.
...
- Create 3-5 test cases covering:
  - Various configurations
  - Edge cases
  - Positive and error condition tests
- Follow existing configuration formats
- Validate across different input sizes and data types

**Step 6: Testing Cycle**
1. Generate test cases using `generate_espdl_op_test_cases` tool
   - Must support all quantized data types: {support_dtype}
2. Build test cases using `build_espdl_op` tool
3. Run tests using `test_espdl_op` tool
...
if __name__ == "__main__":
    ...
    parser.add_argument("--markdown", "-m", help="Markdown文件路径", default="Operators.md")
