...
namespace dl
{
    namespace detect
    {
        ...
        void PedestrianPostprocessor<feature_t>::parse_stage(TensorBase *score, TensorBase *box, const int stage_index)
        {
            ...
            {
                score_threshold_quant = (feature_t) DL_CLIP(dl_esp32p4_round_half_even(this->score_threshold * this->score_threshold / DL_SCALE(score->exponent)), -128, 127);
            }
            else{
                score_threshold_quant = (feature_t) DL_CLIP(dl_esp32p4_round_half_even(this->score_threshold * this->score_threshold / DL_SCALE(score->exponent)), -32768, 32767);
            }
                    ...
                    for (size_t c = 0; c < C; c++) // category number
                    {
                        if (*score_element > score_threshold_quant)
                        {
                            ...
                            result_t new_box = {
                                ...
                                (int)((center_y - dl::math::dfl_integral(box_element_f + 8, 7) * stride_y) / this->resize_scale_y),
                                (int)((center_x + dl::math::dfl_integral(box_element_f + 16, 7) * stride_x) / this->resize_scale_x),
                                (int)((center_y + dl::math::dfl_integral(box_element_f + 24, 7) * stride_y) / this->resize_scale_y)},
                                {0}};
