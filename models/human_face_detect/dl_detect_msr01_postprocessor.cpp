...
namespace dl
{
    namespace detect
    {
        ...
        void MSR01Postprocessor<feature_t>::parse_stage(TensorBase *score, TensorBase *box, const int stage_index)
        {
            ...
            {
                score_threshold_quant = (feature_t) DL_CLIP(dl_esp32p4_round_half_even(dl::math::inverse_sigmoid(this->score_threshold) / DL_SCALE(score->exponent)), -128, 127);
            }
            else{
                score_threshold_quant = (feature_t) DL_CLIP(dl_esp32p4_round_half_even(dl::math::inverse_sigmoid(this->score_threshold) / DL_SCALE(score->exponent)), -32768, 32767);
            }
                            ...
                            if (*score_element > score_threshold_quant)
                            {
                                ...
                                result_t new_box = {
                                    ...
                                    (int)((center_y - (anchor_h >> 1) + anchor_h * box_element[1] * DL_SCALE(box->exponent)) /
                                        this->resize_scale_y),
                                    (int)((center_x + anchor_w - (anchor_w >> 1) + anchor_w * box_element[2] * DL_SCALE(box->exponent)) /
                                        this->resize_scale_x),
                                    (int)((center_y + anchor_h - (anchor_h >> 1) + anchor_h * box_element[3] * DL_SCALE(box->exponent)) /
                                        this->resize_scale_y)},
                                    {0}};
